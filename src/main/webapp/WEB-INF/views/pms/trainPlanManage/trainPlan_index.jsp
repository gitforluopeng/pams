...
<div style="width:100%;height: 98%;border-bottom: solid 1px silver;margin: 0px auto;" class="layui-form" lay-filter="selectdiv">
    ...
    <div id="div-add-toolbar" style="width: 100%;height: 70px;background-color: #F3F3F3;">
        ...
        <div class="layui-inline" style="float: right;margin-top: 15px;margin-right: 40px">
            <button id="addBtn" class="layui-btn layui-btn-danger">创建</button>
        </div>
    </div>
	...
	layui.use(['form','table','laydate'],function(){
		...
		function init_addBtn(){
			addBtn.on('click',function() {
				parent.layer.open({
					...
					shade : 0.8,
					area : [ '50%','55%' ],
					content : 'trainingPlan/trainPlan_add', //iframe的url
					end : function() {
						table.reload('reload',{
							page : {
								curr : 1
		...
		function init_queryBtn(){
			queryBtn.on('click',function(){
				...
				table.reload("reload",{
					url: "trainingPlan/load_trainPlans",
					where: {
		...
		function init_toolClick(){
			table.on('tool(tool)',function(obj){
            	...
            	if(layEvent == "edit"){
					...
					shade : 0.8,
					area : [ '35%', '50%' ],
					content : 'trainingPlan/trainPlan_edit', //iframe的url
					end : function() {
						table.reload('reload');
					}
				})
            		...
            		layer.confirm('确定删除吗?',function(index){
            			$.ajax({
            				type: "post",
            				url: "trainingPlan/delete_trainPlan",
            				data: {
            					"trainPlan.id" : data.id
            				},
